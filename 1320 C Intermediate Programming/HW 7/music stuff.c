...
Node *addNodeToTree(char* filename, Node **currentNode, char* singer_name, int* albums_sold, int number_tour_cities, char** tour_cities_name)
{
...
int city_check(Node *currentNode, char* city)
{
    int count = 0;
    if(!strcmp(*(currentNode)->tour_cities_name, city))
    {
        count++;
    }
    return count;
...
void removeNode(Node **currentNode, int* albums_sold)
{
    if(*currentNode == NULL)
    {
...
int main(int argc, char** argv)
{
    ...
    printf("What city would you like to see?\n");
    fgets(Input, 50, stdin);
    strtok(Input, "\n");
    Node *root = NULL;
    int n = city_check(root, Input);
    
    printf("%s is %d", Input, n);
